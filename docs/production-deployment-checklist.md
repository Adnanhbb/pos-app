...
For the current Hostinger root-domain deployment, do not reuse a package last
generated by the Laragon rehearsal runner. Generate and verify a fresh locked
production package:
...
The Hostinger command builds with
`VITE_API_BASE_URL=https://jawadandbrother.com/api`, `VITE_BASE_PATH=/`,
`VITE_ENABLE_DEV_BACKDOOR=false`, and `VITE_ALLOW_OFFLINE_LOGIN=false`. It
fails if emitted frontend files contain `localhost`, `jawad-bro-rehearsal`, or
`/pos-app/`.
...
Upload the contents of `deployment-package/frontend/` into `public_html/` and
the contents of `deployment-package/api/` into `public_html/api/`. Do not
overwrite the existing server-only `public_html/api/config/private.php`.
...
- For a password diagnosis, run
  `php public_html/api/setup/verify-or-reset-user-password.php` through
  Hostinger SSH. Confirm it reports only safe account state and
  `password_verify` result. Reset requires the explicit `RESET` confirmation
  and uses `PASSWORD_DEFAULT`.
- Delete `public_html/api/setup/verify-or-reset-user-password.php` immediately
  after the supervised verification/reset task.
