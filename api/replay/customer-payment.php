...
try {
    ...
    $result = replayStoredStandaloneCustomerPaymentV1Authorized($pdo, $syncTransactionId, $auth);
    if (($result['success'] ?? false) !== true) {
        $reason = (string) ($result['reason'] ?? 'standalone_customer_payment_replay_failed');
        $status = $reason === 'unauthorized' ? 401 : ($reason === 'not_found' ? 404 : 422);
        error_response('Standalone Customer Payment replay was rejected safely.', $status, [
            'reason' => $reason,
    ...
    success_response([
        ...
        'paymentReplayContract' => 'standaloneCustomerPaymentReplay',
        'payloadVersion' => 1,
        'partyType' => 'customer',
        ...
        'customerId' => isset($result['partyId']) ? (int) $result['partyId'] : null,
