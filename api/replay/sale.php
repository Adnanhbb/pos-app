...
require_once __DIR__ . '/../lib/finalizedSaleReplayV1.php';
...
try {
    ...
    $result = replayStoredFinalizedSaleV1Authorized($pdo, $syncTransactionId, $auth);
    if (($result['success'] ?? false) !== true) {
        $reason = (string) ($result['reason'] ?? 'finalized_sale_replay_failed');
        $status = $reason === 'unauthorized' ? 401 : ($reason === 'not_found' ? 404 : 422);
        error_response('Finalized Sale replay was rejected safely.', $status, [
            'reason' => $reason,
    ...
    success_response([
        ...
        'saleId' => isset($result['saleId']) ? (int) $result['saleId'] : null,
