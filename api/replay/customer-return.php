...
require_once __DIR__ . '/../lib/finalizedCustomerReturnReplayV1.php';
...
try {
    ...
    $result = replayStoredFinalizedCustomerReturnV1Authorized($pdo, $syncTransactionId, $auth);
    if (($result['success'] ?? false) !== true) {
        $reason = (string) ($result['reason'] ?? 'finalized_customer_return_replay_failed');
        $status = $reason === 'unauthorized' ? 401 : ($reason === 'not_found' ? 404 : 422);
        error_response('Finalized Customer Return replay was rejected safely.', $status, [
            'reason' => $reason,
    ...
    success_response([
        ...
        'customerReturnId' => isset($result['customerReturnId']) ? (int) $result['customerReturnId'] : null,
        ...
        'returnBatchesCreated' => isset($result['returnBatchesCreated']) ? (int) $result['returnBatchesCreated'] : 0,
    ]);
