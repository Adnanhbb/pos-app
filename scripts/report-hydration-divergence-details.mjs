...
/*
 * Dev-only detailed hydration divergence diagnostics.
 *
...
function isLikelyDevTestMeta(meta) {
  return containsDevMarker(meta.label) || containsDevMarker(meta.clientId) || containsDevMarker(meta.serverId);
}

function classifyRemoteOnly(meta) {
  if (isLikelyDevTestMeta(meta)) {
    return {
      classification: "likelyDevTestData",
      reason: "Remote-only row has a dev/test marker in safe label or identifier metadata.",
    };
  }

  if (normalizeDeleted(meta.isDeleted)) {
    return {
      classification: "needsManualReview",
      reason: "Remote-only row is soft-deleted; review before deciding whether hydration should create a local deleted row.",
    };
  }

  return {
    classification: "likelyNeedsHydration",
    reason: "Remote-only active row has no obvious dev/test marker.",
  };
}

function classifyLocalOnly(meta) {
  if (isLikelyDevTestMeta(meta)) {
    return {
      classification: "likelyDevTestData",
      reason: "Local-only row has a dev/test marker in safe label or identifier metadata.",
    };
  }

  return {
    classification: "needsManualReview",
    reason: "Local-only row has no matching backend row; do not delete or repair automatically.",
  };
}
...
function classifyDifference() {
  return {
    classification: "needsManualReview",
    reason: "Matched local/backend row differs; inspect before any future hydration apply.",
  };
}

function withClassification(meta, classifier) {
  return { ...compactMeta(meta), ...classifier(meta) };
}
...
function analyzeEntity(config, localRows, backendResult) {
  ...
  for (const localRow of localRows) {
    ...
    if (!remote.row) {
      localOnly.push(withClassification(safeLocalMeta(config, localRow), classifyLocalOnly));
      continue;
    }
    ...
    if (localTime !== null && remoteTime !== null && Math.abs(localTime - remoteTime) > 1000) {
      possibleDivergence.push({
        match: remote.match,
        local: compactMeta(safeLocalMeta(config, localRow)),
        remote: compactMeta(safeRemoteMeta(config, remote.row)),
        ...classifyDifference(),
      });
    }
  ...
  for (const remoteRow of remoteRows) {
    ...
    if (!local.row) {
      remoteOnly.push(withClassification(safeRemoteMeta(config, remoteRow), classifyRemoteOnly));
    }
  }

  const allClassifiedRows = [...localOnly, ...remoteOnly, ...possibleDivergence, ...softDeleteMismatches];
  const classificationCounts = countBy(allClassifiedRows, (row) => row.classification);
  ...
  return {
    ...
    classificationCounts: {
      likelyDevTestData: classificationCounts.likelyDevTestData ?? 0,
      likelyNeedsHydration: classificationCounts.likelyNeedsHydration ?? 0,
      needsManualReview: classificationCounts.needsManualReview ?? 0,
    },
    truncation: {
...
function byEntityDetails(reports) {
  return Object.fromEntries(reports.map((report) => [report.entity, {
    localCount: report.localCount,
    remoteCount: report.remoteCount,
    localOnlyCount: report.totals.localOnly,
    remoteOnlyCount: report.totals.remoteOnly,
    possibleDivergenceCount: report.totals.possibleDivergence,
    softDeleteMismatchCount: report.totals.softDeleteMismatches,
...
async function main() {
  console.log("Dev-only detailed hydration divergence report. Read-only; no hydration or repair is performed.");
  ...
  try {
    ...
    const summary = {
      ...
      classificationTotals: {
        likelyDevTestData: reports.reduce((sum, report) => sum + report.classificationCounts.likelyDevTestData, 0),
        likelyNeedsHydration: reports.reduce((sum, report) => sum + report.classificationCounts.likelyNeedsHydration, 0),
        needsManualReview: reports.reduce((sum, report) => sum + report.classificationCounts.needsManualReview, 0),
      },
      byEntity: byEntityDetails(reports),
    };
