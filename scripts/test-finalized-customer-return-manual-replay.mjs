...
/* Isolated backend verifier for authenticated manual finalized Customer Return v1 replay. */
...
const runId = `finalized-customer-return-v1-${Date.now()}-${Math.random()
  .toString(36)
  .slice(2, 8)}`;
let passed = 0;
let failed = 0;
...
require_once getcwd() . '/api/lib/finalizedCustomerReturnReplayV1.php';

$pdo = get_pdo();
$runId = getenv('FINALIZED_CUSTOMER_RETURN_V1_TEST_RUN_ID') ?: ('finalized-customer-return-v1-' . time());
...
try {
    ...
    $cylinderPayload = make_payload($runId, 'cylinder-ready', $cylinderFixture, true, true, 2, 80, -40);
    $cylinderSyncId = insert_sync_row($pdo, $cylinderPayload);
    $cylinderBefore = snapshot($pdo, $cylinderFixture, $cylinderSyncId);
    $cylinderFirst = replayStoredFinalizedCustomerReturnV1Authorized($pdo, $cylinderSyncId, $auth);
    $cylinderAfterFirst = snapshot($pdo, $cylinderFixture, $cylinderSyncId);
    $cylinderSecond = replayStoredFinalizedCustomerReturnV1Authorized($pdo, $cylinderSyncId, $auth);
    $cylinderAfterSecond = snapshot($pdo, $cylinderFixture, $cylinderSyncId);

    $nonCylinderPayload = make_payload($runId, 'non-cylinder-ready', $nonCylinderFixture, true, false, 3, 20, 0);
    ...
    $nonCylinderReplay = replayStoredFinalizedCustomerReturnV1Authorized($pdo, $nonCylinderSyncId, $auth);
    $nonCylinderAfter = snapshot($pdo, $nonCylinderFixture, $nonCylinderSyncId);

    $unsafePayload = make_payload($runId, 'unsafe', $unsafeFixture, false, true, 2, 80, -40);
    $unsafeSyncId = insert_sync_row($pdo, $unsafePayload);
    $unsafeBefore = snapshot($pdo, $unsafeFixture, $unsafeSyncId);
    $unsafeReplay = replayStoredFinalizedCustomerReturnV1Authorized($pdo, $unsafeSyncId, $auth);
...
function make_payload(string $runId, string $name, array $fixture, bool $ready, bool $withCylinder, float $qty, float $price, float $paid): array {
    ...
    $localSaleId = 93001;
    $localItemId = 93018;
    $localCustomerId = 93007;
    $subtotal = $qty * $price;
    $dues = (float) $fixture['customerOpeningPayable'];
    ...
    $readiness = [
        ...
        'reasons' => $ready ? [] : [[
            ...
            'message' => 'Customer Return fixture intentionally lacks a required backend mapping.',
    ...
    if ($withCylinder) {
        $cylinders[] = [
            'localItemId' => $localItemId,
            'serverItemId' => $serverItemId,
            'localCylinderId' => 93038,
            'serverCylinderId' => $ready ? $fixture['cylinderId'] : null,
            'customerHolding' => [
                'localHoldingId' => 93048,
                'serverHoldingId' => $ready ? $fixture['holdingId'] : null,
    ...
    $contract = [
        ...
        'customer' => [
            ...
            'serverId' => $ready ? $fixture['customerId'] : null,
    ...
    return [
        ...
        'payload' => [
            ...
            'finalizedCustomerReturnReplay' => $contract,
        ],
    ];
}
...
function insert_fixture(PDO $pdo, string $prefix, bool $withCylinder): array {
    $itemName = $prefix . '-mapped-return-item';
    $customerName = $prefix . '-mapped-customer';
    $category = $withCylinder ? 'Gas Cylinder' : 'General';
    $statement = $pdo->prepare("INSERT INTO items (client_id, name, barcode, purchasePrice, retailPrice, discountPrice, wholesalePrice, availableStock, category, brand, minunit, maxunit, ConvQty, is_deleted) VALUES (:client, :name, :barcode, 10, 100, 0, 100, 5, :category, 'Replay Fixture', :minunit, :maxunit, :convQty, 0)");
    $statement->execute([
    ...
    $statement = $pdo->prepare("INSERT INTO customers (client_id, name, mobile, cnic, address, invoices, payable, paid, balance, is_deleted) VALUES (:client, :name, '03000000000', 'customer-return-test', 'customer return replay customer', 0, :payable, 0, :balance, 0)");
    $statement->execute([
        'client' => $prefix . '-customer-client',
        'name' => $customerName,
    ...
    $holdingId = null;
    if ($withCylinder) {
        $statement = $pdo->prepare("INSERT INTO cylinders (itemId, title, qtyInStock, filledCylinders, emptyCylinders, withCustomers, convQty, isDeleted) VALUES (:itemId, :title, 4, 3, 0, 1, 2, 0)");
        $statement->execute(['itemId' => $itemId, 'title' => $prefix . '-cylinder']);
        $cylinderId = (int) $pdo->lastInsertId();
        $statement = $pdo->prepare("INSERT INTO cylinder_customers (cylinderId, cylinderType, customerName, qtyHeld, isDeleted, deletedAt) VALUES (:cylinderId, :cylinderType, :customerName, 1, 0, NULL)");
        $statement->execute([
            'cylinderId' => $cylinderId,
            'cylinderType' => $prefix . '-cylinder',
            'customerName' => $customerName,
        ]);
        $holdingId = (int) $pdo->lastInsertId();
    }
    ...
    return [
        ...
        'customerName' => $customerName,
        'customerOpeningPayable' => $openingPayable,
        'cylinderId' => $cylinderId,
        'holdingId' => $holdingId,
    ];
}
...
function snapshot(PDO $pdo, array $fixture, int $syncId): array {
    ...
    return [
        ...
        'cylinder' => $fixture['cylinderId'] !== null
            ...
            : [],
        'holding' => $fixture['holdingId'] !== null
            ? row($pdo, 'SELECT qtyHeld FROM cylinder_customers WHERE id = :id', ['id' => $fixture['holdingId']])
            : [],
        ...
        'sale' => row($pdo, 'SELECT transactionType, customerId, paid, grandTotal, arrears FROM sales WHERE sync_transaction_id = :id LIMIT 1', ['id' => $syncId]),
        ...
        'payment' => row($pdo, 'SELECT amount, payableSnapshot, balanceSnapshot FROM customer_payments WHERE sync_transaction_id = :id LIMIT 1', ['id' => $syncId]),
...
function cleanup_fixture(PDO $pdo, ?array $fixture): void {
    ...
    $pdo->prepare('DELETE FROM customer_payments WHERE customerId = :id OR client_transaction_id LIKE :pattern')->execute(['id' => $fixture['customerId'], 'pattern' => $pattern]);
    $pdo->prepare('DELETE FROM sale_items WHERE sale_id IN (SELECT id FROM sales WHERE customerId = :id OR client_transaction_id LIKE :pattern)')->execute(['id' => $fixture['customerId'], 'pattern' => $pattern]);
    $pdo->prepare('DELETE FROM sales WHERE customerId = :id OR client_transaction_id LIKE :pattern')->execute(['id' => $fixture['customerId'], 'pattern' => $pattern]);
...
function runPhpHarness() {
  const result = spawnSync(findPhpBinary(), ["-r", phpTestCode()], {
    ...
    env: {
      ...process.env,
      FINALIZED_CUSTOMER_RETURN_V1_TEST_RUN_ID: runId,
    },
    windowsHide: true,
...
check("PHP finalized Customer Return replay harness completed", result, (value) => value.ok === true, "Harness failed.");
if (result.ok === true) {
  check(
    "ready Customer Return commits exactly once",
    result,
    (value) =>
      ...
      value.cylinderAfterFirst?.sale?.transactionType === "Customer Return",
    "Ready Customer Return did not commit one Customer Return and one item."
  );
  check(
    "server item id drives Customer Return stock increase",
    result,
    (value) =>
      nearlyEqual(value.cylinderBefore?.itemStock, 5) &&
      nearlyEqual(value.cylinderAfterFirst?.itemStock, 7),
    "Mapped backend stock was not increased."
  );
  check(
    "return batch is created from safe returnBatch metadata",
    result,
    (value) =>
      Number(value.cylinderAfterFirst?.createdBatchCount) === 1 &&
      Number(value.cylinderAfterFirst?.createdBatch?.id) !== 93028 &&
      Number(value.cylinderAfterFirst?.createdBatch?.itemId) ===
        Number(value.cylinderFixture?.itemId) &&
      Number(value.cylinderAfterFirst?.createdBatch?.sourceSaleId) ===
        Number(value.cylinderAfterFirst?.saleId) &&
      nearlyEqual(value.cylinderAfterFirst?.createdBatch?.qtyPurchased, 2) &&
      nearlyEqual(value.cylinderAfterFirst?.createdBatch?.balance, 2),
    "Customer Return batch creation mismatch."
  );
  check(
    "customer accounting and negative payment ledger mirror local Customer Return outcome",
    result,
    (value) =>
      Number(value.cylinderAfterFirst?.customer?.invoices) === 1 &&
      nearlyEqual(value.cylinderAfterFirst?.customer?.payable, 340) &&
      nearlyEqual(value.cylinderAfterFirst?.customer?.paid, -40) &&
      nearlyEqual(value.cylinderAfterFirst?.customer?.balance, 380) &&
      Number(value.cylinderAfterFirst?.paymentCount) === 1 &&
      nearlyEqual(value.cylinderAfterFirst?.payment?.amount, -40) &&
      nearlyEqual(value.cylinderAfterFirst?.payment?.payableSnapshot, 340) &&
      nearlyEqual(value.cylinderAfterFirst?.payment?.balanceSnapshot, 380),
    "Customer accounting or negative payment ledger mismatch."
  );
  check(
    "cylinder Customer Return applies customerHoldingToEmpty exactly",
    result,
    (value) =>
      nearlyEqual(value.cylinderAfterFirst?.cylinder?.filledCylinders, 3) &&
      nearlyEqual(value.cylinderAfterFirst?.cylinder?.emptyCylinders, 1) &&
      nearlyEqual(value.cylinderAfterFirst?.cylinder?.withCustomers, 0) &&
      nearlyEqual(value.cylinderAfterFirst?.cylinder?.qtyInStock, 4) &&
      nearlyEqual(value.cylinderAfterFirst?.holding?.qtyHeld, 0),
    "Cylinder Customer Return did not move holding to empty exactly."
  );
  check(
    "second Customer Return replay is idempotent",
    result,
    (value) =>
    ...
    "Second Customer Return replay changed business state."
  );
  check(
    "unsafe Customer Return is rejected without mutation",
    result,
    (value) =>
    ...
    "Unsafe Customer Return replay changed state or was accepted."
  );
  check(
    "non-cylinder Customer Return works without cylinder metadata",
    result,
    (value) =>
      ...
      Number(value.nonCylinderAfter?.paymentCount) === 0 &&
      nearlyEqual(value.nonCylinderBefore?.itemStock, 5) &&
      nearlyEqual(value.nonCylinderAfter?.itemStock, 8) &&
      Number(value.nonCylinderAfter?.createdBatchCount) === 1 &&
      Array.isArray(value.nonCylinderAfter?.cylinder) &&
      value.nonCylinderAfter.cylinder.length === 0,
    "Non-cylinder Customer Return replay did not commit safely."
  );
  check(
    "local ids are not backend mutation targets",
    result,
    (value) =>
      Number(value.cylinderFixture?.itemId) !== 93018 &&
      Number(value.cylinderFixture?.customerId) !== 93007 &&
      Number(value.cylinderFixture?.cylinderId) !== 93038 &&
      Number(value.cylinderFixture?.holdingId) !== 93048 &&
      Number(value.cylinderAfterFirst?.createdBatch?.id) !== 93028 &&
  ...
  check(
    ...
    (rows) =>
      ...
      rows.some(
        (row) =>
          row.event_type === "finalized_customer_return_v1_replay_completed"
      ) &&
      rows.some(
...
check(
  ...
  syncEngineSource,
  (source) =>
    source.includes('payload.payload?.returnMode === "customer"') &&
    source.includes("assertReadyFinalizedCustomerReturnReplay(item.payload)") &&
    source.includes(
      "transactionApi.replayFinalizedCustomerReturn(item.payload.clientTransactionId)"
    ),
  "Frontend manual router does not call narrow Customer Return replay endpoint."
);
check(
  "transaction API targets narrow Customer Return replay endpoint",
  transactionApiSource,
  (source) =>
    source.includes(
      'apiClient.post("/replay/customer-return.php", { clientTransactionId })'
    ),
  "Narrow Customer Return endpoint is not wired."
);
check(
  "Customer Return endpoint requires replay auth before execution",
  endpointSource,
  (source) =>
    source.includes("require_replay_request_auth($pdo)") &&
    source.includes("replayStoredFinalizedCustomerReturnV1Authorized"),
  "Customer Return endpoint auth guard is missing."
);
check(
